...
public class Trn001ServiceImpl implements Trn001Service {
    ...
    // GET DETAIL
    @Override
    public BaseResponseDto<?> getVehicleDetail(String key) {
        try {
            Trn001DetailVo data = trn001Dao.getVehicleDetail(key);
            if (data == null) {
                return BaseResponseDto.error("Data kendaraan tidak ditemukan", null);
            }
            return BaseResponseDto.success("Data kendaraan ditemukan", data);
        } catch (Exception e) {
            return BaseResponseDto.error("Gagal mengambil detail kendaraan : " + e.getMessage(), null);
        }
    }
